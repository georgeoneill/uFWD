function [L,D] = ufwd_eeg_lgainmat(D,Is,channels)
...
try
    ...
    for ind = 1:numel(forward)
        ...
        %-Forward computation
        %------------------------------------------------------------------
        [vol, sens] = ufwd_prepare_vol_sens(vol, sens, 'channel', forward(ind).channels);
        nvert = size(vert, 1);
        ...
        if ~isequal(ufwd_headmodeltype(vol), 'interpolate')
            Gxyz = zeros(length(forward(ind).channels), 3*nvert);
            for i = 1:nvert
                if siunits
                    Gxyz(:, (3*i - 2):(3*i))  = ufwd_compute_leadfield(vert(i, :), sens, vol,...
            ...
            if siunits
                Gxyz = ufwd_compute_leadfield(vert, sens, vol, 'dipoleunit', 'nA*m', 'chanunit', units);
            else
                Gxyz = ufwd_compute_leadfield(vert, sens, vol);
            end
        end
